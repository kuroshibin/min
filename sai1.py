...
helpMessage ="""  BOT มิน BOTป้องกัน+BOTแท็คชื่อ
รายชื่อคำสั่งแต่ละคำ+รายละเอียด

1. มิน help     คำสั่งเพื่อเรียกดูรายการคำสั่งทั้งหมด
2. แท็คชื่อ     คำสั่งแท็คชื่อคนในห้องแบบทั้งหมด เฉพาะคนในห้อง1-500คน
3. Bot     คำสั่งแท็คชื่อคนทั้งห้อง เฉพาะห้องที่มีคน150คนลง
4. SAI     คำสั่งแท็คชื่อเฉพาะคนในห้องที่มี150คนลง
5. Me     คำสั่ง สั่งให้บอทส่งคอนแทคหรือข้อมูลติดต่อเราในห้อง

หมวดสั่งบอทให้ส่งสติ้กเกอร์
...
Bots=[mid,Amid,Bmid,Cmid,"ua1cb6e845fe8f2646fe8a5c598a5c5911841fa"]
admin=["ua1cb6e845fe8f2646fe8a5c598a5c5911841fa"]
wait = {
    'contact':True,
    'autoJoin':True,
    ...
    'timeline':False,
    'autoAdd':False,
    'message':"ขอบคุณ สนใจบอทติดต่อแชท ไอดี socool290",
    "lang":"JP",
    "comment":"ขอบคุณ สนใจบอทติดต่อแชท ไอดี socool290",
    "commentOn":False,
    "commentBlack":{},
    "wblack":False,
    "dblack":False,
		...
		for ticket_id in n_links:
			if wait["atjointicket"] == True:
              ...
              if msg.from_ in admin:
                ...
                md = ""
                if wait["Protectjoin"] == True: md+="￼￼􂤁􀅣>􏿿 Block Join\n"
                else: md+=" Block Join Off\n"
                if wait["Protectgr"] == True: md+="􂤁􀅣>􏿿￼ Block Group\n"
                else: md+=" Block Group Off\n"
                if wait["Protectcancl"] == True: md+="￼􂤁􀅣>􏿿 Cancel All Invited\n"
                else: md+=" Cancel All Invited Off\n"
                if wait["contact"] == True: md+="􂤁􀅣>􏿿 Contact    : on\n"
                else: md+=" Contact    : off\n"
                if wait["autoJoin"] == True: md+="􂤁􀅣>􏿿 Auto join : on\n"
                else: md +=" Auto join : off\n"
                if wait["autoCancel"]["on"] == True:md+="􂤁􀅣>􏿿￼ Group cancel :" + str(wait["autoCancel"]["members"]) + "\n"
                else: md+= " Group cancel : off\n"
                if wait["leaveRoom"] == True: md+="􂤁􀅣>􏿿 Auto leave    : on\n"
                else: md+=" Auto leave : off\n"
                if wait["timeline"] == True: md+="􂤁􀅣>􏿿 Share   : on\n"
                else:md+=" Share   : off\n"
                if wait["autoAdd"] == True: md+="􂤁􀅣>􏿿 Auto add : on\n"
                else:md+=" Auto add : off\n"
                if wait["commentOn"] == True: md+="􂤁􀅣>􏿿 Comment : on\n"
                else:md+=" Comment : off\n"
...
#-----------------------------------------------
    ...
    #-------------Fungsi Tag All Start---------------#
            elif msg.text in ["SAI","sai","ทรายแทก","ทรายแท็คชื่อ","ฺBot"]:
...
#-----------------------------------------------
    ...
    ki.sendText(msg.to,"Bot กัน+แท็คชื่อคนทั้งห้อง ระบบกันลบ ลบ1บอทจะลบ กันเชิญ กันการเปิดลิ้งค์ กันการยกเลิกค้างค้าง และระบบอีกมากมาย ชุด 4+1 3+1 5+1 6+1 10+1 15+1 20+1 28+1 30+1 32+1 สามารถเลืออกบบอทเพิ่มบอทป้องได้แบบไม่จำกัด ราคาแล้วแต่จำนวนบอท  จ่ายรอบเดีวไม่มีรายเดือน")
    ki.sendText(msg.to," สนใจ Bot  ติดต่อ ทราย
􀔃􀄎LINE messenger􏿿http://line.me/ti/p/~saibot01
􀔃􀄎LINE messenger􏿿http://line.me/ti/p/~bot_botv13
0902853778

ดาว
􀔃􀄎LINE messenger􏿿http://line.me/ti/p/~devilgirl.
􀔃􀄪phone􏿿0908927220ทราย
􀔃􀄎LINE messenger􏿿http://line.me/ti/p/~saibot01
0902853778
